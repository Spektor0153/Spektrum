...
import styles from "./../LeftMenu/leftMenu.module.css";
import {NavLink, Link} from "react-router-dom";
import Badge from "react-bootstrap/Badge";
import {compose} from "redux";
import {connect} from "react-redux";
import {socket} from "./../../socket/SocketMy";
import {newMessageCountMenuChange} from "./../LeftMenu/LeftMenuAction";
                    ...
                    <ul>
                        <li>
                            <NavLink className={styles.nav_a} to='/'><span
                                className={`${styles.icon} ${styles.home_icon} `}></span></NavLink>
                        </li>
                        <li>
                            <NavLink className={styles.nav_a} to="/friends"><span
                                className={`${styles.icon} ${styles.friends_icon} `}></span></NavLink>
                        </li>
                        <li>
                            <NavLink className={styles.nav_a} to='/dialogs'><span
                                className={`${styles.icon} ${styles.dialogs_icon} `}>{this.props.countNewM>0?
                        ...
                        <li>
                            <NavLink className={styles.nav_a} to="/news"><span
                                className={`${styles.icon} ${styles.news_icon} `}></span></NavLink>
                        </li>
                    </ul>
                </div>
            </div>
...
export default compose(connect(mapStateToProps, {newMessageCountMenuChange}))(MobileMenu)